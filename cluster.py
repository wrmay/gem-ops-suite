...
def startLocator(processName):
	...
	#these are optional
	if clusterDef.hasLocatorProperty(processName,'bind-address'):
		cmdLine.append('--bind-address={0}'.format(clusterDef.locatorProperty(processName, 'bind-address')))
...
def startServerCommandLine(processName):
	...
	if clusterDef.hasDatanodeProperty(processName,'server-bind-address'):
		cmdLine.append('--server-bind-address={0}'.format(clusterDef.datanodeProperty(processName, 'server-bind-address')))
